...
//设备系列
typedef enum _JDeviceProduct
{
	JDP_CloudSEE,		//云视通一代产品
	JDP_CloudSEE2,		//云视通2.0产品
...
typedef struct _JCLanDeviceInfo
{
	...
	JCDeviceType nDeviceType;
	int nChannelNum;
	int nSubStreamNum;
	JDeviceProduct nProduct;
}
...
*			success to callback to return connectoin ID,connection result will be return in event callback,Fail to call to return JCSDK_INVALID_LINKVALUE 
*/ 
JCSDK_API JCLink_t JCSDK_CALLTYPE JCSDK_Connect2(JDeviceProduct nProduct, char *szServer, int nPort, int nChannel,
	int nStream, char *szNetUser, char *szPwd, JBOOL bNumOrIP, LPVOID pUserData);
